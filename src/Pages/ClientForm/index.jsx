...
const ClientForm = ({ handleLogout }) => {
    ...
    const fetchAuthToken = async () => {
        try {
            const response = await fetch(`${process.env.REACT_APP_CIFRUC_URL}/login`, {
                method: 'GET',
                headers: {
    ...
    const handleDocumentSearch = async (event) => {
        if (event.key === 'Enter' || event.key === 'Tab') {
            ...
            try {
                ...
                const response = await fetch(`${process.env.REACT_APP_CIFRUC_URL}/document/search`, {
                    method: 'POST',
                    headers: {
                ...
                if (data.document.establishments && data.document.establishments.length > 0) {
                ...
                if (cif.length === 10 && /^[0-9]+$/.test(cif)) {
                    setErrorMessage("El Documento de Identificación ingresado es Incorrecto o No Existe.");
                    ...
                    setWarningMessage("El Documento de Identificación ingresado es un Pasaporte.");
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (customerCode) {
                response = await apiClient.patch(`/clients/${customerCode}`, payloadUpdate);
            ...
            if (error.response?.data?.message === 'CLIENT_ARCHIVED') {
                ...
                setIsSuccessVisible(false);
                setIsErrorVisible(false);
            } else if (error.response?.data?.message.startsWith('CLIENT_ALREADY_EXISTS')) {
                setIsInfoVisible(false);
                setIsSuccessVisible(false);
                setIsErrorVisible(true);
                setErrorMessage('El cliente ya existe.');
    ...
    return (
        <div className="home-container">
            ...
            <div className="moduleNew-form-container">
                ...
                <div className="basic-info-form-grid">
                    <div className="basic-info-form-group">
                        <label style={{ color: errors.codcli ? 'red' : 'inherit' }}>Código de Cliente</label>
                        <input
                            type="text"
                            placeholder="Código"
                            value={codcli}
                    ...
                    <div className="basic-info-form-group">
                        <label style={{ color: errors.cif ? 'red' : 'inherit' }}>Documento de Identificación</label>
                        <input
                            type="text"
                    ...
                    <div className="basic-info-form-group">
                        <label style={{ color: errors.nomcli ? 'red' : 'inherit' }}>Razón Social</label>
                        <input
                            type="text"
                    ...
                    <div className="basic-info-form-group">
                        <label style={{ color: errors.tipoCif ? 'red' : 'inherit' }}>Tipo Documento de Identificación</label>
                        <input
                            type="text"
                        ...
                        <input
                            ...
                            value={tipoCif}
                        />
                        {errors.tipoCif && <p className="error-message">{errors.tipoCif}</p>}
                    </div>
                    <div className="basic-info-form-group">
                        <label style={{ color: errors.direc1 ? 'red' : 'inherit' }}> Dirección</label>
                        <input
                            type="text"
                    ...
                    <div className="basic-info-form-group">
                        <label style={{ color: errors.tlf1 ? 'red' : 'inherit' }}>Teléfono</label>
                        <input
                            type="text"
            ...
            <SuccessNotification
                message={customerCode ? "El Cliente se ha Actualizado Correctamente." : "El CLiente se ha Creado Correctamente."}
